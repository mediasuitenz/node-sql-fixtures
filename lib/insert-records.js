...
function buildInsertPromise(knex, tableName, records, unique, showWarning) {
  ...
  return getInsertableRecords(knex, tableName, insertRecords, unique).then(function(insertableRecords) {
    ...
    if (isPostgres(knex)) {
      insertFn = (records) => knex(tableName).returning('*').insert(records);
    } else {
      insertFn = (records) => insertRecordsSerially(knex, tableName, records, showWarning);
    }

    return Promise.all(_.chunk(insertRecords, 500).map(insertFn)).then(function(insertResults) {
        return assembleFinalResult(_.union(...insertResults));
    });
  });
}
